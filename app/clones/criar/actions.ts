...
import { startFluxLoraFastTraining } from "@/lib/fal-service"
...
export async function createClone(formData: FormData): Promise<{ clone?: Clone; error?: string }> {
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    ...
    const { error: uploadError } = await supabase.storage.from("training_images").upload(filePath, file)
    if (uploadError) {
      console.warn(
  ...
  const initialCloneData = {
    ...
    training_images: storedImageUrls.length > 0 ? storedImageUrls : null,
